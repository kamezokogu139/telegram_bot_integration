...
class GoalCallbackTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_matching_session_goal_callback_sends_postback(self):
        pending = _pending()
        ...
        with (
            ...
            await bot.goal_callback(_update("goal_pick_current_1"), context)

        self.assertEqual(calls, [("click-12345", "secure", "dep", 2, "108")])
    ...
    async def test_callback_without_pending_does_not_edit_result_message(self):
        update = _update("goal_pick_current_1")
        context = SimpleNamespace(user_data={}, bot=_FakeBot())

        await bot.goal_callback(update, context)

        self.assertEqual(update.callback_query.messages, [])
        self.assertTrue(update.callback_query.answers)

    def test_goal_keyboard_callback_data_includes_session_id(self):
        keyboard = bot._build_goals_keyboard(_pending()["goals"], "session123")

        self.assertEqual(
            [row[0].callback_data for row in keyboard.inline_keyboard],
            ["goal_pick_session123_0", "goal_pick_session123_1", "goal_cancel_session123"],
        )
